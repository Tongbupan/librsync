 ...
 *
 * \brief MD5 message-digest accumulator.
 *
 * \sa rs_mdfour(), rs_mdfour_begin(), rs_mdfour_update(),
 * rs_mdfour_result()
 */
typedef struct rs_mdfive rs_mdfive_t;

#define RS_STRONG_SUM_LEN 16

typedef unsigned int rs_weak_sum_t;
typedef unsigned char rs_strong_sum_t[RS_STRONG_SUM_LEN];

void rs_mdfive(unsigned char *out, void const *in, size_t n);
int rs_mdfive_begin(rs_mdfive_t *md);
int rs_mdfive_update(rs_mdfive_t *md, const void *data, size_t len);
int rs_mdfive_result(rs_mdfive_t *md, unsigned char *out);
...
#define RS_DEFAULT_STRONG_LEN 16
...
LIBRSYNC_API
rs_result rs_sig_file(MFILE *old_file, MFILE *sig_file,
                      size_t block_len, size_t strong_len, rs_stats_t *); 

LIBRSYNC_API
rs_result rs_loadsig_file(MFILE *, rs_signature_t **, rs_stats_t *);

LIBRSYNC_API
rs_result rs_file_copy_cb(void *arg, rs_long_t pos, size_t *len, void **buf);

LIBRSYNC_API
rs_result rs_delta_file(rs_signature_t *, MFILE *new_file, MFILE *delta_file, rs_stats_t *);

LIBRSYNC_API
rs_result rs_patch_file(MFILE *basis_file, MFILE *delta_file, MFILE *new_file, rs_stats_t *);

LIBRSYNC_API
rs_result rs_reverse_delta(rs_signature_t *new_sig, MFILE *basic_file, size_t *match_list, rs_stats_t *);
